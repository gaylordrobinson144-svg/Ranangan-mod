...
/// </summary>
/// <remarks>
/// Content located in <c>moddirectory/installer/(hd/lv/addon)</c> will be moved to its appropriate directory.
/// </remarks>
sealed class GameContentInstaller
{
	/// <summary>
	...
	private static bool VerifyFilesExist(string sourceMapsDirectory, string? entFilesName, IEnumerable<string> mapFileNames)
	{
		...
		bool everythingNeededExists = !missingFiles.Any() && !missingDirectories.Any();
	...
	public void Install(string rootDirectory, IEnumerable<GameInstallData> games)
	{
		...
		foreach (var game in games)
		{
			...
			Console.WriteLine("Updating last modified time for node graphs");
            ...
            Console.WriteLine("Finishing updating node graphs");
		}
	}
}
